...
void WritePathsRefined(ReconstructionStructure& RS, vector<SkelGraph>& SG, vector< vector<int_type_t> >& paths, vector< vector<double> >& path_diameters,
		string outfile){
	...
	// this color does not show up in Blender.
	//colors.push_back(c);
	...
	for (int_type_t i = 0, in = paths.size(); i < in; i++){
		...
		if (paths[i].size() > 0){
			{
				two_item_path = paths[i].size() == 2;
				//if (paths[i].size() == 3){
				//cout << "paths size is  " << paths[i].size() << endl;
				//}
				for (int_type_t j = 1, jn= paths[i].size() - 1 + two_item_path; j < jn; j++){
					if (j == 1){
						RS.ReturnXYZIndicesFromIndex(SG[paths[i][j-1]].grid_id, x_index, y_index, z_index);
						start_point[0] = RS.BB[0][0] + (x_index)*RS.inter_voxel_distance + RS.inter_voxel_distance/2.0;
						start_point[1] = RS.BB[0][1] + (y_index)*RS.inter_voxel_distance + RS.inter_voxel_distance/2.0;
					...
					if (two_item_path){
						//cout << "two item path and entered " << endl;

						next_point[0] = middle_point[0];
						next_point[1] = middle_point[1];
						next_point[2] = middle_point[2];
					...
					if (j == 1){
						...
						if (two_item_path || paths[i].size() == 3){
							//cout << "Tidy up at the end two item" << endl;
